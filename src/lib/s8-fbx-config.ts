...
const DIGEST_KEYS = ["S8_SANDBOX_POLICY_SHA256", "S8_BLENDER_EXECUTABLE_SHA256"] as const;
const CONFIG_KEYS = [...PATH_KEYS, ...DIGEST_KEYS] as const;

type S8PathEnvironment = Partial<Record<(typeof CONFIG_KEYS)[number], string | undefined>>;
...
export function readS8RuntimeConfig(environment: S8PathEnvironment = process.env as S8PathEnvironment): S8WorkerConfig | undefined {
  const values = CONFIG_KEYS.map((key) => environment[key]);
  if (values.every((value) => value === undefined || value === "")) return undefined;
  const missing = CONFIG_KEYS.filter((key) => typeof environment[key] !== "string" || environment[key]!.length === 0);
  if (missing.length) fail("S8_RUNTIME_CONFIG_INVALID", missing[0]!);
  for (const key of DIGEST_KEYS) {
    if (!/^[0-9a-f]{64}$/u.test(environment[key]!)) fail("S8_RUNTIME_CONFIG_INVALID", key);
  }
  return {
    ...
    sandboxPolicySha256: environment.S8_SANDBOX_POLICY_SHA256!,
