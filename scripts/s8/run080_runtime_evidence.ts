...
function requiredEnv(name: string): string {
  ...
  if (!value) throw new Error(`RUN080_MISSING_ENV_${name}`);
  return value;
}
...
function classifyCell(label: string, root: string, custody: "none" | "root-acl", identity: "none" | "hosted", surfaces: string[], appError: string, meta: Record<string, string>, stdout: string, stderr: string, artifact: Buffer | null): Cell {
  ...
  return {
    ...
    stdout: boundedText(stdout), stderr: boundedText(stderr), artifact,
  };
}
...
function makeSources(): { s6: S6ToS7Handoff; s7: S7ToS8Handoff } {
  ...
  const object = {
    objectId: "run080-object",
    identityKey: "run080-identity",
    parentObjectId: null,
    objectType: "box",
    role: "furniture",
    label: "run080-object",
    ...
    provenance: { kind: "user_confirmed_design_decision", sourceRef: "run080", sourceFingerprint: hash, acceptedByUser: true, note: null },
  };
  const s6 = {
...
abort() {
  printf 'STATUS=%s\nERROR=%s\nWORK_SOURCE=%s\nSNAPSHOT_PRE=%s\nSNAPSHOT_POST=%s\n' "$2" "$1" "\${work_source:-UNKNOWN}" "$pre" "$post" > "$meta"
  exit "$2"
}
snapshot_path() {
...
[[ -n "$work_source" && $separator -gt 0 ]] || abort ARGUMENT_SHAPE_INVALID 70
snapshot_path "$work_source" "$pre" || abort WORK_SNAPSHOT_FAILED 71
custody_before="$(/usr/bin/stat -c '%d:%i:%u:%g:%a' -- "$work_source")"
if [[ "\${S8_RUN080_SHIM_CUSTODY:-none}" == "root-acl" ]]; then
  runner_uid="\${S8_RUN080_RUNNER_UID:?}"
...
if [[ "\${S8_RUN080_SHIM_SURFACES:-NO}" == "YES" ]]; then
  ...
  done < "$S8_RUN080_SHIM_SURFACE_FILE"
  while IFS=$'\t' read -r mask_source mask_target; do
    [[ -n "\${mask_source:-}" ]] || continue
    final_args+=(--ro-bind "$mask_source" "$mask_target")
  done < "\${S8_RUN080_SHIM_MASK_FILE:-/dev/null}"
  final_args+=("\${after[@]}")
elif ((\${#identity_args[@]} > 0)); then
  final_args=("\${identity_args[@]}" "\${args[@]}")
fi
if [[ "\${S8_RUN080_SHIM_SANITIZE_ENV:-NO}" == "YES" ]]; then
  mapfile -t policy_args < "$S8_RUN080_SHIM_ENV_FILE"
  set +e
  /usr/bin/env -i "\${policy_args[@]}" /usr/bin/sudo -n /usr/bin/bwrap "\${final_args[@]}" > "$stdout_path" 2> "$stderr_path"
  status=$?
  set -e
else
  set +e
  /usr/bin/sudo -n /usr/bin/bwrap "\${final_args[@]}" > "$stdout_path" 2> "$stderr_path"
  status=$?
  set -e
fi
printf 'STATUS=%s\nWORK_SOURCE=%s\nSNAPSHOT_PRE=%s\nSNAPSHOT_POST=%s\nCUSTODY_BEFORE=%s\nCUSTODY_AFTER=%s\n' "$status" "$work_source" "$pre" "$post" "$custody_before" "$custody_after" > "$meta"
...
function makeConfig(paths: { blenderRoot: string; blender: string; writer: string; runner: string; validator: string; shim: string; root: string; sha: string }): S8WorkerConfig {
  return {
...
function makeCellRunner(options: {
  ...
  const shimEnvironment: Record<string, string> = {
    S8_RUN080_SHIM_DIAG_BASE: base,
    S8_RUN080_SHIM_CUSTODY: options.custody,
    S8_RUN080_SHIM_IDENTITY: options.identity,
    S8_RUN080_SHIM_SURFACES: options.surfaceIds.length ? "YES" : "NO",
    S8_RUN080_SHIM_SURFACE_FILE: surfaceFile,
    S8_RUN080_SHIM_MASK_FILE: maskFile,
    S8_RUN080_SHIM_ENV_FILE: envFile,
    S8_RUN080_SHIM_SANITIZE_ENV: options.envPolicy ? "YES" : "NO",
    S8_RUN080_RUNNER_UID: String(options.runnerUid),
  };
  let appError = "";
  ...
  const writer = withEnvironment(invocationEnv, () => {
    try {
      ...
      appError = error instanceof AppError ? error.code : error instanceof Error ? error.name : "RUN080_UNKNOWN_ERROR";
      return null;
    }
  });
...
function surfaceForId(id: string): Surface {
  ...
  if (!value) throw new Error(`RUN080_UNKNOWN_SURFACE_${id}`);
  return value;
}
...
function applyCustody(path: string, runnerUid: number): void {
  ...
  if (chown.status !== 0) throw new Error("RUN080_DIRECT_CUSTODY_CHOWN_FAILED");
  const chmod = sudoCommand(["/usr/bin/chmod", "0770", "--", path]);
  if (chmod.status !== 0) throw new Error("RUN080_DIRECT_CUSTODY_CHMOD_FAILED");
  const acl = `u::rwx,u:${runnerUid}:rwx,g::---,m::rwx,o::---`;
  if (sudoCommand(["/usr/bin/setfacl", "--no-mask", "--set", acl, "--", path]).status !== 0) throw new Error("RUN080_DIRECT_ACCESS_ACL_FAILED");
  if (sudoCommand(["/usr/bin/setfacl", "--no-mask", "--default", "--set", acl, "--", path]).status !== 0) throw new Error("RUN080_DIRECT_DEFAULT_ACL_FAILED");
  const after = statSync(path);
  if (before.dev !== after.dev || before.ino !== after.ino) throw new Error("RUN080_DIRECT_CUSTODY_IDENTITY_CHANGED");
}

function directBwrap(options: {
  root: string; identity: "none" | "hosted"; custody: "none" | "root-acl"; surfaces: string[]; masks: Array<{ source: string; target: string }>; envPolicy: EnvPolicy; runner: string; validator?: string; target: string; targetArgs: string[]; work: string;
}): DirectResult {
  ...
  const command = runCommand("/usr/bin/env", ["-i", ...envLines(options.envPolicy), "/usr/bin/sudo", "-n", "/usr/bin/bwrap", ...args]);
  return { ...command, receiptCode: receiptCode(command.stdout), pass: command.status === 0 && receiptCode(command.stdout) === 0 };
}

function runValidatorSurface(options: { label: string; root: string; identity: "none" | "hosted"; custody: "none" | "root-acl"; artifact: Buffer; surfaceIds: string[]; masks: Array<{ source: string; target: string }>; envPolicy: EnvPolicy; runner: string; validator: string; runnerUid: number; diagnosticsRoot: string }): DirectResult {
  const work = mkdtempSync(join(options.root, "s8-run080-validator-"));
  ...
  const result = directBwrap({ root: options.root, identity: options.identity, custody: options.custody, surfaces: options.surfaceIds, masks: options.masks, envPolicy: options.envPolicy, runner: options.runner, validator: options.validator, target: "/runtime/process-runner", targetArgs: ["--address-space-bytes", String(S8_LIMITS.validatorMemoryBytes), "--file-bytes", String(S8_LIMITS.validatorTempBytes), "--timeout-ms", String(S8_LIMITS.validatorTimeoutMs), "--stdout-bytes", String(S8_LIMITS.readbackBytes), "--stderr-bytes", String(S8_LIMITS.stderrBytes), "--max-children", "0", "--", "/runtime/validator", "/work/artifact.fbx"], work });
...
function resolveLibrary(name: string): string {
  ...
  if (!match?.[1]) throw new Error(`RUN080_LIBRARY_NOT_FOUND_${name}`);
...
function printTopologySummary(cells: Cell[], mountDomains: { runnerTemp: MountInfo | null; topLevel: MountInfo | null; carrier: MountInfo | null }, limitations: string[]): void {
  ...
  console.log(`ACCEPTED_CARRIER_MOUNT_DOMAIN=${JSON.stringify(mountDomains.carrier)}`);
  console.log(`MOUNT_DOMAIN_DIFFERENCE=${JSON.stringify({ runnerTempVsTopLevel: !sameMountDomain(mountDomains.runnerTemp, mountDomains.topLevel), topLevelVsCarrier: !sameMountDomain(mountDomains.topLevel, mountDomains.carrier) })}`);
...
function makeMountDomains(runnerTemp: string, topLevel: string, carrier: string): { runnerTemp: MountInfo | null; topLevel: MountInfo | null; carrier: MountInfo | null } {
  return { runnerTemp: mountInfoForPath(runnerTemp), topLevel: mountInfoForPath(topLevel), carrier: mountInfoForPath(carrier) };
}

function main(): void {
  if (process.platform !== "linux" || process.arch !== "x64") throw new Error("RUN080_PLATFORM_HOLD");
  const runId = requiredEnv("S8_RUN080_RUN_ID");
  const runAttempt = requiredEnv("S8_RUN080_RUN_ATTEMPT");
  if (!/^\d+$/u.test(runId) || !/^\d+$/u.test(runAttempt)) throw new Error("RUN080_RUN_ID_INVALID");
  const runnerTemp = realpathSync(requiredEnv("S8_RUN080_RUNNER_TEMP"));
  const carrier = realpathSync(requiredEnv("S8_RUN080_CARRIER"));
  const paths = {
    blenderRoot: realpathSync(requiredEnv("S8_RUN080_BLENDER_ROOT")),
    blender: realpathSync(requiredEnv("S8_RUN080_BLENDER")),
    writer: realpathSync(requiredEnv("S8_RUN080_WRITER")),
    runner: realpathSync(requiredEnv("S8_RUN080_RUNNER")),
    validator: realpathSync(requiredEnv("S8_RUN080_VALIDATOR")),
    shim: "",
    root: "",
    sha: realpathSync(requiredEnv("S8_RUN080_BLENDER")),
  };
  paths.sha = sha256File(paths.blender);
  const runnerUid = process.getuid?.() ?? -1;
  const runnerGid = process.getgid?.() ?? -1;
  if (runnerUid < 1 || runnerGid < 1) throw new Error("RUN080_RUNNER_IDENTITY_INVALID");
  const diagnosticsRoot = mkdtempSync(join(runnerTemp, `s8-run080-diagnostics-${runId}.${runAttempt}-`));
  ...
  console.log(`RUN080_PAYLOAD_SHA256=${createHash("sha256").update(payload).digest("hex")}`);
  ...
  const surfaceSource = (id: string, target: string): Surface | null => {
    ...
    if (!surfaceCandidatePolicy(surface, literal)) throw new Error(`RUN080_SURFACE_INVALID_${id}`);
    return surface;
  };
  ...
  const topRoot = `/s8-run080-private-${runId}.${runAttempt}`;
  if (existsSync(topRoot) || lstatSync(topRoot, { throwIfNoEntry: false })) throw new Error("RUN080_TOP_LEVEL_ROOT_NOT_FRESH");
  const install = sudoCommand(["/usr/bin/install", "-d", "-o", String(runnerUid), "-g", String(runnerGid), "-m", "0700", "--", topRoot]);
  if (install.status !== 0) throw new Error("RUN080_TOP_LEVEL_ROOT_CREATE_FAILED");
  let controlRoot = "";
  const cells: Cell[] = [];
  const limitations: string[] = [];
  let topRootCreated = true;
  try {
    controlRoot = mkdtempSync(join(runnerTemp, `s8-run080-control-${runId}.${runAttempt}-`));
    chmodSync(controlRoot, 0o700);
    const mountDomains = makeMountDomains(runnerTemp, topRoot, carrier);
    if (!sameMountDomain(mountDomains.topLevel, mountDomains.carrier)) throw new Error("RUN080_TOP_LEVEL_MOUNT_DOMAIN_MISMATCH");
    ...
    console.log(`ACCEPTED_CARRIER_ROOT=${JSON.stringify(pathMetadata(carrier))}`);
    ...
    cells.push(t0);
    printCell(t0);
    if (t0.bindSourceReached !== "NO") throw new Error("RUN080_T0_CONTROL_MOVED");
    ...
    const executeSurfaceCase = (label: string, ids: string[], policy: EnvPolicy, masks: Array<{ source: string; target: string }> = []): { writer: Cell; validator: DirectResult | null } => {
      ...
      const validator = runValidatorSurface({ label: `${label}_VALIDATOR_SURFACE`, root: selected!.root, identity: selected!.identity, custody: selected!.custody, artifact: writer.artifact, surfaceIds: ids, masks: [], envPolicy: policy, runner: paths.runner, validator: paths.validator, runnerUid, diagnosticsRoot });
    ...
    for (const path of [emptyInterpreter, emptyLibc, emptyLibm]) { writeFileSync(path, Buffer.alloc(0), { mode: 0o444 }); chmodSync(path, 0o444); }
    ...
    console.log(`VALIDATOR_MINIMUM_SURFACES=interpreter:${validatorElf.interpreter};libc:${libc};libm:${libm};system:${minimumIds.join(",")}`);
    ...
    const runActualValidatorPass = (artifact: Buffer, policy: EnvPolicy): boolean => withEnvironment(policy, () => {
      try {
        const result = runS8NativeValidator(artifact, makeConfig({ ...configPaths, root: selected!.root }));
    ...
    for (const key of ENV_KEYS) {
      ...
      if (trial.writer.artifact) validatorPass = runActualValidatorPass(trial.writer.artifact, without);
    ...
    const finalEnvValidator = finalEnv.writer.artifact ? runActualValidatorPass(finalEnv.writer.artifact, minimumEnv) : false;
    for (const key of ENV_KEYS) {
      console.log(`KEY=${key}`);
    ...
    const hostilePolicy: EnvPolicy = { ...minimumEnv, S8_TEST_PARENT_SECRET_A: "controlled-a", S8_TEST_PARENT_SECRET_B: "controlled-b", PATH: "/run080-hostile-path", HOME: "/run080-hostile-home", LD_PRELOAD: "/run080-hostile-preload", LD_LIBRARY_PATH: "/run080-hostile-library", PYTHONPATH: "/run080-hostile-pythonpath", PYTHONHOME: "/run080-hostile-pythonhome" };
    const probeScript = "set -eu; test -z \"${S8_TEST_PARENT_SECRET_A+x}\"; test -z \"${S8_TEST_PARENT_SECRET_B+x}\"; test \"${PATH-}\" != /run080-hostile-path; test \"${HOME-}\" != /run080-hostile-home; test -z \"${LD_PRELOAD+x}\"; test -z \"${LD_LIBRARY_PATH+x}\"; test -z \"${PYTHONPATH+x}\"; test -z \"${PYTHONHOME+x}\"; printf 'ENV_NEGATIVE_CONTROLS=PASS\\n'";
    const probeEnv = withEnvironment(hostilePolicy, () => directBwrap({ root: selected.root, identity: selected.identity, custody: selected.custody, surfaces: minimumIds, masks: [], envPolicy: minimumEnv, runner: paths.runner, target: "/usr/bin/bash", targetArgs: ["-ceu", probeScript], work: selected.root }));
    ...
    const surfaceComplete = finalSurface.writer.pass && finalSurface.validator?.pass === true;
    ...
    if (topRootCreated && existsSync(topRoot) && !lstatSync(topRoot).isSymbolicLink()) {
      ...
      else console.log(`RUN080_CLEANUP_RETAINED_CHILDREN=${children.length}`);
    }
...
try {
  ...
  const code = error instanceof AppError ? error.code : error instanceof Error ? error.message : "RUN080_UNKNOWN_FAILURE";
  console.error(`RUN080_FATAL_BOUNDARY=${code}`);
  process.exitCode = 1;
}
