...
const VARIANTS: Record<string, string[]> = {
  ...
  "QA-016": ["material-vs-uncertain", "violation-vs-compliant", "uncertainty-vs-compliant", "two-material", "max-height-shared", "persisted-reject"],
  ...
  "REPAIR-017": ["access-reduction", "eligibility", "operation-hash", "repair-input-hash", "prompt-objective"],
...
function fixtureFor(testId: string, variantId: string): string {
  ...
  if (testId === "QA-016") return "Real local shared access.open-sides and applicable geometry.max-height requirement/design-rule observations through production reduction and persisted-graph validation.";
  ...
  if (testId === "ROUTE-001") return "Complete nine-case unknown-project matrix through the production S2 dispatcher with one valid absent UUID, real supporting IDs, valid bodies/headers including bounded multipart upload, per-case safe 404 facts and zero repository/object/provider side effects.";
