...
# S8 B3 private transform serialization. This module is generated from the
# pinned upstream exporter and changes only the accepted source-rigid model
# transform boundary. It intentionally has no ordinary transform fallback.
def _s8_transform_values(ob_obj, s8_transform_table):
    if not isinstance(s8_transform_table, dict):
        raise RuntimeError("S8_TRANSFORM_TABLE_REQUIRED")
    binding = s8_transform_table.get(ob_obj.name)
    if not isinstance(binding, dict):
        raise RuntimeError("S8_TRANSFORM_TABLE_MISSING")
    local_translation_ticks = binding.get("localTranslationTicks")
    source_euler_microdegrees = binding.get("sourceEulerMicrodegrees")
    unit_scale_ticks = binding.get("unitScaleTicks")
    if (
        not isinstance(local_translation_ticks, (list, tuple))
        or len(local_translation_ticks) != 3
        or not isinstance(source_euler_microdegrees, (list, tuple))
        or len(source_euler_microdegrees) != 3
        or not isinstance(unit_scale_ticks, (list, tuple))
        or len(unit_scale_ticks) != 3
    ):
        raise RuntimeError("S8_TRANSFORM_TABLE_INVALID")
    for values in (local_translation_ticks, source_euler_microdegrees, unit_scale_ticks):
        if any(isinstance(value, bool) or not isinstance(value, int) for value in values):
            raise RuntimeError("S8_TRANSFORM_TABLE_INVALID")
    scale = tuple(value / 10000000000.0 for value in unit_scale_ticks)
    if scale != (1.0, 1.0, 1.0):
        raise RuntimeError("S8_SOURCE_SCALE_NOT_RIGID")
    return (
        tuple(value / 1000000.0 for value in local_translation_ticks),
        tuple(value / 1000000.0 for value in source_euler_microdegrees),
        scale,
    )
...
def fbx_data_object_elements(root, ob_obj, scene_data, s8_transform_table):
    """
    ...
    # S8 B3: source-rigid values are the sole transform serialization input.
    loc, rot, scale = _s8_transform_values(ob_obj, s8_transform_table)
...
def fbx_objects_elements(root, scene_data, s8_transform_table):
    """
    ...
    for ob_obj in scene_data.objects:
        if ob_obj.is_dupli:
            continue
        fbx_data_object_elements(objects, ob_obj, scene_data, s8_transform_table)
        for dp_obj in ob_obj.dupli_list_gen(scene_data.depsgraph):
            if dp_obj not in scene_data.objects:
                continue
            fbx_data_object_elements(objects, dp_obj, scene_data, s8_transform_table)
...
def save_single(operator, scene, depsgraph, filepath="",
    ...
    with encode_bin.FBXElem.enable_multithreading_cm():
        ...
        # Actual data.
        fbx_objects_elements(root, scene_data, s8_transform_table)
