...
import { S8_BLENDER_PIN, S8_LIMITS, S8_WRITER_RECEIPT_VERSION, s8Sha256 } from "./s8-fbx-profile";
...
function parseReceipt(bytes: Buffer, payloadSha256: string, writerSha256: string, executableSha256: string, artifact: Buffer): S8WriterReceipt {
...
export function runS8BlenderWriter(payloadBytes: Buffer, config: S8WorkerConfig): S8WriterResult {
  ...
  try {
    ...
    const receipt = parseReceipt(receiptBytes, s8Sha256(payloadBytes), writerSha256, executableSha256, artifact);
