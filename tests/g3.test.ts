...
function lockText(lockPath: string): string | null {
  try {
...
test("two live repository instances cannot mutate canonical state concurrently", () => {
  const root = mkdtempSync(join(tmpdir(), "swooshz-design-lock-exclusive-"));
  const owner = new JsonRepository(root, {
    processId: 8001,
    isProcessAlive: (processId) => processId === 8001,
  });
  ...
  try {
    owner.transact((state) => {
      assert.throws(
        () => contender.transact(() => "must not mutate"),
        (error: any) => {
          blocked = error?.code === "PERSISTENCE_BUSY";
          return blocked;
        },
      );
      addLockTestProject(state, "owner commit");
      return state.projects.length;
    });
    assert.equal(blocked, true);
    assert.equal(new JsonRepository(root).state().projects.length, 1);
  } finally {
    cleanup(root);
  }
});

test("a live v2 owner cannot be stolen after canonical claim", () => {
...
test("every ownership-publication interruption phase recovers without an anonymous canonical lock", () => {
  const phases = [
    "candidate-created",
  ...
  for (const phaseToInterrupt of phases) {
    ...
    try {
      ...
      const afterClaim =
        phaseToInterrupt === "canonical-claimed" ||
        phaseToInterrupt === "before-acquisition-return";
      if (afterClaim) {
        assert.ok(canonical, "canonical claim must be visible after the atomic claim phase");
        const record = JSON.parse(canonical as string);
        assert.equal(record.protocol, "swooshz-repository-lock-v2");
        assert.equal(typeof record.ownerToken, "string");
        assert.equal(record.processId, 8201);
      } else {
        assert.equal(canonical, null, "pre-claim interruption must not expose canonical ownership");
      }

      const recovered = new JsonRepository(root, {
        processId: 8202,
        isProcessAlive: () => false,
        lockWaitMs: 250,
      });
      recovered.transact((state) => {
        addLockTestProject(state, "recovered after " + phaseToInterrupt);
        return state.projects.length;
      });
      assert.equal(lockText(lockPath), null);
      assert.equal(
        readdirSync(root).some((name) => name.endsWith(".candidate")),
        false,
      );
    } finally {
      cleanup(root);
...
test("malformed historical lock artefacts and incomplete candidates do not permanently wedge the repository", () => {
  const root = mkdtempSync(join(tmpdir(), "swooshz-design-lock-malformed-"));
  const lockPath = join(root, "state.json.lock");
  try {
    writeFileSync(lockPath, '{"ownerToken":"truncated"');
    writeFileSync(lockPath + ".legacy.candidate", "");
    ...
    recovered.transact((state) => {
      addLockTestProject(state, "malformed recovery");
      return state.projects.length;
    });
