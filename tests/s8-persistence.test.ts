...
test("publication phases are monotonic and source movement always fences", () => {
  let value = record();
  for (const phase of ["claim", "private_staging", "independent_validation", "source_claim_recheck", "immutable_promotion", "commit"] as const) value = advanceS8Publication(value, "c", "o", phase, value.heartbeatAtMs + 1);
