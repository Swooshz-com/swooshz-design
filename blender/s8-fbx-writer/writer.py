...
def transform_binding(item: dict[str, Any]) -> dict[str, Any]:
    ...
    validate_matrix_ticks(item.get("matrixTicks"))
    return {
...
def _validate_object_record(item: Any, material_by_id: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    _text(item.get("identityKey"), "S8_OBJECT_INVALID", 4096)
...
def _snapshot_depsgraph_instance(instance: Any, expected: dict[str, Any]) -> str:
    is_instance = bool(getattr(instance, "is_instance", False))
    candidate = _original_object(getattr(instance, "object", None))
    candidate_name = getattr(candidate, "name", None)
    if is_instance:
        fail("S8_EXPORT_INSTANCE_FORBIDDEN")
    if not isinstance(candidate_name, str) or not any(candidate is value for value in expected.values()):
        fail("S8_EXPORT_INSTANCE_FORBIDDEN")
...
def audit_evaluated_membership(admission: dict[str, Any], state: dict[str, Any], depsgraph: Any) -> None:
    ...
    if instances is not None:
        seen_names = [
            _snapshot_depsgraph_instance(instance, expected)
            for instance in instances
        ]
        if len(seen_names) != len(set(seen_names)) or set(seen_names) != set(expected):
            fail("S8_EXPORT_OBJECT_SET_INVALID")
