...
export function s2CandidatePreviewPath(projectId: string, qaRunId: string, candidateId: string): string {
  return apiPath(projectId, "/s2/qa-runs/" + qaRunId + "/candidates/" + candidateId + "/preview");
}
...
export function orderS2Candidates(candidates: readonly Candidate[]): Candidate[] {
  return candidates.slice().sort((left, right) => left.candidateIndex - right.candidateIndex);
}
...
export function S2ReferencesScreen({ projectId, sourceGenerationSetId }: { projectId: string; sourceGenerationSetId: string | null }) {
  ...
  return <main><p className="muted">Swooshz Design / S2 visual inputs</p><h1>Reference inputs</h1><p>Upload optional visual references and logos, then preserve their exact order.</p><p role="note">Accepted formats: PNG, JPEG, WebP. Per-file maximum: 8 MiB. Ordered reference maximum: 6 references. Ordered logo maximum: 2 logos.</p><p className="disclaimer">S2 is visual/design QA only; it is not engineering, structural, venue, fabrication, legal, cost, or approval confirmation.</p>{error ? <p className="error">{error}</p> : null}{draft ? <section className="panel"><p className="muted">Revision {draft.revision} / {draft.status}</p><label>Upload kind<select value={kind} disabled={draft.status === "frozen"} onChange={(event) => setKind(event.target.value as "reference" | "logo")}><option value="reference">Reference</option><option value="logo">Logo</option></select></label><label>Image<input type="file" accept=".png,.jpg,.jpeg,.webp,image/png,image/jpeg,image/webp" disabled={draft.status === "frozen"} onChange={(event) => setFile(event.target.files?.[0] ?? null)} /></label><button type="button" disabled={busy || !file || draft.status === "frozen"} onClick={() => void upload()}>Upload</button><div className="candidate-grid">{draft.assets.filter((asset) => asset.status === "ready").map((asset) => <label className="candidate" key={asset.id}><input type="checkbox" checked={selected.has(asset.id)} disabled={busy || draft.status === "frozen"} onChange={() => toggle(asset)} />{asset.kind} / {asset.width}x{asset.height} / {Math.round(asset.normalizedBytes / 1024)} KiB</label>)}</div><h2>Ordered references</h2><ol>{draft.referenceAssetIds.map((id, index) => <li key={id}>{id} <button type="button" disabled={busy || draft.status === "frozen" || index === 0} onClick={() => { const asset = draft.assets.find((item) => item.id === id); if (asset) reorder(asset, -1); }}>Up</button><button type="button" disabled={busy || draft.status === "frozen" || index === draft.referenceAssetIds.length - 1} onClick={() => { const asset = draft.assets.find((item) => item.id === id); if (asset) reorder(asset, 1); }}>Down</button></li>)}</ol><h2>Ordered logos</h2><ol>{draft.logoAssetIds.map((id, index) => <li key={id}>{id} <button type="button" disabled={busy || draft.status === "frozen" || index === 0} onClick={() => { const asset = draft.assets.find((item) => item.id === id); if (asset) reorder(asset, -1); }}>Up</button><button type="button" disabled={busy || draft.status === "frozen" || index === draft.logoAssetIds.length - 1} onClick={() => { const asset = draft.assets.find((item) => item.id === id); if (asset) reorder(asset, 1); }}>Down</button></li>)}</ol><button type="button" disabled={busy || draft.status === "frozen" || !sourceGenerationSetId} onClick={() => void client.bind(draft.revision)}>Run S2 QA</button></section> : null}</main>;
}
