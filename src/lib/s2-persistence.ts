...
function selectedAssetProjection(state: StoreState, ids: readonly string[], kind: "reference" | "logo", projectId: string): RepairAssetProjection[] {
...
function validateObservationSet(result: S2QaCandidateResult | S2ReQaResult, input: S2InputVersion): void {
  ...
  const isTerminal = ["pass", "warning", "material_fail"].includes(result.status);
  ...
  if (result.status === "queued" || result.status === "running" || isUnavailable) {
    ...
    input.canonicalRequirements.forEach((expected, index) => {
      const observed = result.requirementObservations[index];
        ...
        observed.expectedCount === expected.expectedCount && equalJson(observed.expectedValue, expected.expectedValue), "candidate.requirement-order");
      if (expected.expected !== "exact_count") ensure(observed.observedCount === null, "candidate.requirement-observed-count");
      if (expected.expected === "exact_count" && (observed.observed === "present" || observed.observed === "absent") && observed.confidence >= S2_CONFIDENCE_THRESHOLD) {
    ...
    const reduced = reduceS2Findings(input, result.requirementObservations, result.designObservations);
    ensure(equalIds(result.materialFindingIds, reduced.materialFindingIds), "candidate.material-findings");
    ensure(equalIds(result.warningFindingIds, reduced.warningFindingIds), "candidate.warning-findings");
    ensure(equalIds(result.uncertainFindingIds, reduced.uncertainFindingIds), "candidate.uncertain-findings");
    if (result.status === "pass") ensure(result.verdict === "PASS" && reduced.verdict === "PASS", "candidate.pass-tuple");
    if (result.status === "warning") ensure(result.verdict === "WARNING" && reduced.verdict === "WARNING", "candidate.warning-tuple");
    if (result.status === "material_fail") ensure(result.verdict === "MATERIAL_FAIL" && reduced.verdict === "MATERIAL_FAIL", "candidate.material-tuple");
  }
...
function validateRepairState(repair: S2RepairAttempt, run: S2QaRun, input: S2InputVersion, state: StoreState, operations: Map<string, S2Operation>, derived: Map<string, S2DerivedCandidate>, reQa: Map<string, S2ReQaResult>): void {
  ...
  const referenceAssets = selectedAssetProjection(state, input.referenceAssetIds, "reference", input.projectId);
  const logoAssets = selectedAssetProjection(state, input.logoAssetIds, "logo", input.projectId);
  ensure(repair.repairInputHash === canonicalRepairInputHash(input, source, repair.eligibleFindingIds, referenceAssets, logoAssets), "repair.input-hash");
  const prompt = renderS2RepairPrompt(input, source, repair.eligibleFindingIds, referenceAssets, logoAssets, repair.repairInputHash);
  ensure(repairPromptHash(prompt) === repair.repairPromptHash, "repair.prompt-hash");
